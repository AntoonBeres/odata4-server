...
import { ServiceDocument } from 'odata4-service-document';
import { Edm as Metadata } from 'odata4-metadata';
import * as ODataParser from 'odata4-parser';
...
export class ODataServerBase extends Transform {
  ...
  _transform(chunk: any, _?: string, done?: (...params: any[]) => void) {
    ...
    this.serverType.execute(chunk).then(
      (result) => {
        this.push(result);
  ...
  static addController(controller: typeof ODataController, isPublic?: boolean, elementType?);
  static addController(controller: typeof ODataController, entitySetName?: string, elementType?);
  static addController(controller: typeof ODataController, entitySetName?: string | boolean, elementType?) {
...
export function createODataServer(
  server: typeof ODataServer,
  path?: string | RegExp | number,
  port?: number | string,
  hostname?: string,
): http.Server | express.Router {

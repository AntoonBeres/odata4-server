...
import { Token, TokenType } from 'odata4-parser/lib/lexer';
...
const createODataContext = function (context: ODataHttpContext, entitySets, server: typeof ODataServer, resourcePath, processor) {
  ...
  resourcePath.navigation.forEach((baseResource, i): string | void => {
      ...
      if (prevResource) {
        ...
        if (typeof returnType == 'function') {
          ...
          returnType = entitySet
            ? entitySet + (returnTypeName.indexOf('Collection') == 0 ? selectContextPart : selectContextPart + '/$entity')
            : returnTypeName;
    ...
    if (baseResource.type == TokenType.EntityCollectionNavigationProperty) {
      prevResource = baseResource;
      odataContext += '/' + baseResource.name;
      prevType = baseResource.key
        ? Edm.getType(prevType, baseResource.name, server.container)
        : server.getController(<Function>Edm.getType(prevType, baseResource.name, server.container));
    ...
    if (
      baseResource.type == TokenType.PrimitiveProperty ||
...
const fnCaller = function (this: any, fn, params) {
  params = params || {};
  const fnParams = getFunctionParameters(fn);
...
const jsPrimitiveTypes = [Object, String, Boolean, Number, Date];

const writeMethods = ['delete', 'post', 'put', 'patch'];
...
const defaultHandlers = [ODataGeneratorHandlers.GeneratorHandler, ODataGeneratorHandlers.PromiseHandler, ODataGeneratorHandlers.StreamHandler];
...
export class ODataProcessor extends Transform {
      ...
      if (!(chunk instanceof Buffer)) {
        ...
        try {
          ...
          if (chunk instanceof Object) {
            ...
            const deferConvert = this.__convertEntity(
              entity,
              chunk,
              this.elementType || this.ctrl.prototype.elementType,
              this.resourcePath.includes,
              this.resourcePath.select,
            );
  ...
  private __EntityCollectionNavigationProperty(part: NavigationPart): Function {
    return async (result) => {
      try {
        ...
        const method = writeMethods.indexOf(this.method) >= 0 && partIndex < this.resourcePath.navigation.length - 1 ? 'get' : this.method;
    ...
    return async (result) => {
      try {
        return new Promise(async (resolve, reject) => {
          ...
          let fn =
            odata.findODataMethod(this.ctrl, `${this.method}/${part.name}`, prevPart.key || []) ||
  ...
  private __read(ctrl: typeof ODataController, part: any, params: any, data?: any, filter?: string | Function, elementType?: any, include?, select?) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const method =
          writeMethods.indexOf(this.method) >= 0 && this.resourcePath.navigation.indexOf(part) < this.resourcePath.navigation.length - 1
            ? 'get'
            : this.method;
        ...
        if (typeof filter == 'string' || !filter) {
          ...
          if (include && filter && include.query && !include.query.$filter) {
            ...
            queryString = Object.keys(include.query)
              .map((p) => `${p}=${include.query[p]}`)
              .join('&');
          } else if (
            ...
            queryString =
              Object.keys((include || this).query)
                .map((p) => {
                  if (p == '$filter' && filter) {
        ...
        switch (method) {
          ...
          case 'patch':
            ...
            if (typeParam) {
              params[typeParam] = (
                body['@odata.type'] || `${(<any>ctrl.prototype.elementType).namespace}.${(<any>ctrl.prototype.elementType).name}`
              ).replace(/^#/, '');
            }
            if (bodyParam) {
          ...
          if (isStream(result) && include) {
            include.streamPromise.then((result) => {
              (<Promise<ODataResult>>ODataRequestResult[method](result)).then((result) => {
                ...
                return this.__appendODataContext(
                  result,
            ...
            return (<Promise<ODataResult>>ODataRequestResult[method](result)).then((result) => {
              if (!this.streamStart && writeMethods.indexOf(this.method) < 0 && !result.body) return reject(new ResourceNotFoundError());
              try {
                if (elementType) result.elementType = elementType;
    ...
    return (data) =>
      new Promise(async (resolve, reject) => {
        try {
          ...
          if (
            Edm.isActionImport(this.serverType, part.name) ||
            schemas.some((schema) =>
              schema.entityContainer.some((container) => container.actionImports.some((actionImport) => actionImport.name == part.name)),
            )
          ) {
            isAction = true;
      ...
      new Promise(async (resolve, reject) => {
        try {
          ...
          if (entityBoundOp) {
            ...
            if (
              Edm.isAction(elementType, boundOpName) ||
              schemas.some((schema) =>
                schema.actions.some(
                  (action) =>
                    action.name == boundOpName &&
                    action.isBound &&
                  ...
                  (action) =>
                    ...
                    action.parameters.some(
                      (parameter) =>
                        ...
                        parameter.type == 'Collection(' + ((<any>elementType).namespace + '.' + (<any>elementType).name) + ')',
                    ),
                ),
              )
            ) {
          ...
          if (boundOp == expOp) {
            ...
            if (elementType && boundOpName == '$value' && typeof elementType == 'function' && Edm.isMediaEntity(elementType)) {
              ...
              return expResult.then(
                (expResult) =>
                  (<Promise<ODataResult>>(
                    (boundOpName == '$ref' && this.method != 'get' ? ODataResult.NoContent : ODataRequestResult[this.method])(
                      expResult,
                      typeof expResult == 'object' ? 'application/json' : 'text/plain',
                    )
                  )).then((result) => {
  ...
  private async __appendLinks(ctrl, elementType, context, body, result?) {
      ...
      try {
        if (keys.length == 1) {
          id = await Edm.escape(
            ...
            Edm.getURLSerializer(elementType, keys[0], Edm.getType(elementType, keys[0], this.serverType.container), this.serverType.container),
          );
        } else {
          id = (
            await Promise.all(
              keys.map(
                async (it) =>
                  `${it}=${await Edm.escape(
                    ...
                    Edm.getURLSerializer(elementType, it, Edm.getType(elementType, it, this.serverType.container), this.serverType.container),
                  )}`,
              ),
            )
          ).join(',');
    ...
    if (entitySet && typeof id != 'undefined') {
      ...
      if (odata.findODataMethod(ctrl, 'put', keys) || odata.findODataMethod(ctrl, 'patch', keys)) {
  ...
  private async __appendODataContext(result: any, ctrlType: Function, includes?, select?) {
    ...
    const elementType = (result.elementType =
      jsPrimitiveTypes.indexOf(result.elementType) >= 0 || result.elementType == String || typeof result.elementType != 'function'
        ? ctrlType
        : result.elementType);
    if (typeof result.body == 'object' && result.body) {
      ...
      if (!result.body['@odata.context']) {
        ...
        if (result.body.value && Array.isArray(result.body.value)) {
          context.value = [];
          await Promise.all(
            result.body.value.map((entity, i) =>
              (async (entity, i) => {
  ...
  private __setODataType(context, elementType) {
    ...
    if (containerType) {
      context['@odata.type'] = `#${
        odata.getNamespace(Object.getPrototypeOf(this.serverType.container).constructor, containerType) ||
        this.serverType.container['namespace'] ||
        elementType.namespace ||
        this.serverType.namespace
      }.${containerType}`;
    } else {
  ...
  private async __include(include: ResourcePathVisitor, select, context, prop, ctrl: typeof ODataController, result, elementType) {
    ...
    if (typeof result[prop] == 'object') {
      ...
      const params = {};
      let stream: ODataStreamWrapper, streamPromise: Promise<object>;
      if (isCollection) {
  ...
  private __enableStreaming(part: NavigationPart) {
    this.streamEnabled =
      part == this.resourcePath.navigation[this.resourcePath.navigation.length - 1] ||
  ...
  private async __applyParams(container: any, name: string, params: any, queryString?: string | Token, result?: any, include?) {
    ...
    const elementType = (result && result.elementType) || (this.ctrl && this.ctrl.prototype.elementType) || null;
    if (queryParam) {
      ...
      if (typeof queryAst == 'string') {
        ...
        const queryType =
          lastNavigationPath.type == 'QualifiedEntityTypeName'
    ...
    if (filterParam) {
      ...
      const resourceFilterAst =
        this.resourcePath.ast.value.query &&
        this.resourcePath.ast.value.query.value.options &&
        this.resourcePath.ast.value.query.value.options.find((t) => t.type == TokenType.Filter);

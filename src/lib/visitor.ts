...
export class ResourcePathVisitor {
  ...
  protected async VisitExpand(node: Token, context: any, type: any) {
    await Promise.all(
      node.value.items.map(async (item) => {
  ...
  protected async VisitSimpleKey(node: Token, _: any, type: any) {
    ...
    const deserializer = Edm.getURLDeserializer(type, node.value.key, node[ODATA_TYPE], this.serverType.container);
  ...
  protected async VisitCompoundKey(node: Token, _: any, type: any) {
    this.path += '(\\(';
    const lastNavigationPart = this.navigation[this.navigation.length - 1];
    lastNavigationPart.key = await Promise.all<KeyValuePair>(
      node.value.map(async (pair, i) => {
        ...
        const deserializer = Edm.getURLDeserializer(type, pair.value.key.value.name, node[ODATA_TYPE], this.serverType.container);
        if (typeof deserializer == 'function') value = await deserializer(value);
        return {
  ...
  protected async VisitBoundFunctionCall(node: Token, context: any, type: any) {
    ...
    await Promise.all(
      node.value.params.value.map(async (param, i) => {
  ...
  protected async VisitFunctionParameter(node: Token, context: any) {
    const edmParam = context.parameters.find((p) => p.name.toString() == [node.value.name.value.name].toString());
    const deserializer = (edmParam && Edm.getURLDeserializer(node[ODATA_TYPE], edmParam.name, edmParam.type, this.serverType.container)) || ((_) => _);

...
export function createMetadataJSON(server: typeof ODataServer) {
  ...
  const getAllOperations = function (target: Function): any {
    const ops = Edm.getOperations(target);
    let ret = {};
  ...
  const resolveTypeDefinition = (elementType, prop, namespace, getType = Edm.getType, getTypeName = Edm.getTypeName) => {
    const defType = getType(elementType, prop, server.container);
  ...
  const resolveEnumType = (elementType, prop, namespace, enumType?, enumName?) => {
    ...
    if (!enumName) {
      ...
      enumDefinition = {
        name: enumName,
        underlyingType:
          Edm.getTypeName(containerType, `${enumNamespace}.${enumName}`, server.container) ||
          Edm.getTypeName(containerType, enumName, server.container) ||
          'Edm.Int32',
  ...
  const resolveOperation = (elementType, prop, namespace, isBound?, boundType?) => {
    ...
    if (Edm.isTypeDefinition(elementType, prop)) {
      ...
      const definitionContainer = Edm.isEntityType(type) ? 'entityType' : 'complexType';
      const resolvedType = resolveType(type, elementType, prop);
    ...
    const returnType = Edm.getReturnTypeName(elementType, prop, server.container);
    const parameters = Edm.getParameters(elementType, prop).map((p) => {
      const param = Object.assign({}, p);
      if (typeof param.type != 'string') {
        const resolvedContainerType = server.container.resolve(param.type) || param.type.name;
        if (Edm.isTypeDefinition(param.type)) {
          param.type = resolveTypeDefinition(containerType, resolvedContainerType, namespace, Edm.getType, (): string =>
            Edm.getTypeName(containerType, resolvedContainerType, server.container),
          );
          ...
          if (typeof param.type == 'function') {
            const definitionContainer = Edm.isEntityType(param.type) ? 'entityType' : 'complexType';
            const resolvedType = resolveType(
              param.type,
              elementType,
              prop,
              undefined,
              resolvedContainerType.indexOf('.') > 0 ? resolvedContainerType : `${param.type.namespace || namespace}.${resolvedContainerType}`,
            );
            if (resolvedType && !resolvedType.schema[definitionContainer].filter((et) => et.name == resolvedType.definition.name)[0]) {
              resolvedType.schema[definitionContainer].push(resolvedType.definition);
            }
  ...
  const resolveImport = (operationSchema, i, definition, type) => {
    ...
    const containerName = server.prototype[i].containerName || 'Default';
  ...
  const resolveType = (elementType, parent, prop?, baseType?, paramTypeName?) => {
    ...
    const namespace = elementType.namespace || parent.namespace || server.namespace;
    ...
    const properties: string[] = Edm.getProperties(elementType.prototype);
    properties.forEach((prop) => {
      ...
      if (Edm.isTypeDefinition(elementType, prop)) {
        ...
        const type = Edm.getType(elementType, prop, server.container);
        if (typeof type == 'function') {
          const definitionContainer = Edm.isEntityType(elementType, prop) ? 'entityType' : 'complexType';
          const resolvedType = resolveType(type, elementType, prop);
          if (resolvedType && !resolvedType.schema[definitionContainer].filter((et) => et.name == resolvedType.definition.name)[0]) {
            resolvedType.schema[definitionContainer].push(resolvedType.definition);
          }
    ...
    const operations: any = getAllOperations(elementType);
    Object.keys(operations).forEach((operation) => {
      const namespace = operations[operation].prototype[operation].namespace || elementType.namespace || parent.namespace || server.namespace;
      const elementTypeNamespace = elementType.namespace || parent.namespace || server.namespace;
      ...
      if (Edm.isFunction(operations[operation], operation)) {
        operationSchema.function.push(
          resolveOperation(operations[operation], operation, namespace, true, `${elementTypeNamespace}.${elementType.name}`),
        );
      }
    ...
    if (baseType && baseType != Object && Edm.getProperties(baseType.prototype).length > 0) {
      ...
      const definitionContainer = Edm.isEntityType(baseType) ? 'entityType' : 'complexType';
      if (resolvedType && !resolvedType.schema[definitionContainer].filter((et) => et.name == resolvedType.definition.name)[0]) {
        resolvedType.schema[definitionContainer].push(resolvedType.definition);
      }
    }
    const children = Edm.getChildren(elementType);
    children.forEach((child) => {
      const resolvedType = resolveType(child, elementType, prop, elementType);
      const definitionContainer = Edm.isEntityType(child) ? 'entityType' : 'complexType';
          ...
          if (Edm.isFunction(operations[operation], operation)) {
            operationSchema.function.push(
              resolveOperation(
                ...
                namespace,
                true,
                `Collection(${elementTypeNamespace}.${ctrl.prototype.elementType.name})`,
              ),
            );
          }
  ...
  propNames.forEach((i) => {
    if (i != '$metadata' && server.prototype[i]) {
      const operationNamespace = server.prototype[i].namespace || server.namespace;

...
function typeDecoratorFactory(type: string): any {
  const decorator = function (target?: any, targetKey?: string, parameterIndex?: number) {
    ...
    if (baseType != Object && getProperties(baseType.prototype).length > 0) {
      ...
      const children = Reflect.getOwnMetadata(EdmChildren, baseType) || [];
    ...
    if (typeof parameterIndex == 'number') {
      ...
      const param: any = existingParameters.filter((p) => p.name == paramName)[0];
      if (param) {
        param.type = type;
      } else {
...
export function Collection(elementType: Function): Decorator {
  return function (target, targetKey, parameterIndex?: number) {
    if (typeof parameterIndex == 'number') {
      ...
      const elementTypeName = Reflect.getMetadata(EdmType, element, EdmType);
...
export function getTypeName(target: Function, propertyKey: string, container?: ContainerBase): string {
  let type =
    Reflect.getMetadata(EdmType, target) ||
    Reflect.getMetadata(EdmType, target.prototype, propertyKey) ||
    Reflect.getMetadata(EdmType, target.prototype);
  let elementType =
    Reflect.getMetadata(EdmElementType, target) ||
    Reflect.getMetadata(EdmElementType, target.prototype, propertyKey) ||
    Reflect.getMetadata(EdmElementType, target.prototype);
  ...
  if (container && container instanceof ContainerBase) {
    ...
    if (containerType) {
      ...
      let namespace =
        odata.getNamespace(containerPrototype.constructor, containerType) || (<any>(containerPrototype.constructor || target)).namespace || 'Default';
  ...
  if (typeof type == 'string' && type != 'Collection' && type.indexOf('.') < 0) {
    for (let i = 0; i < EdmContainer.length; i++) {
      const containerType = EdmContainer[i];
      const namespace = (<any>containerType).namespace || (<any>target).namespace || 'Default';
      const containerTypeName = (<any>containerType).name;
...
export function getType(target: Function, propertyKey: string, container?: ContainerBase): Function | string {
  ...
  if (typeof elementType == 'string') {
    if (container && container instanceof ContainerBase) {
      const containerType = container.resolve(type);
      if (containerType) {
        const namespace =
          (<any>containerType).namespace || Object.getPrototypeOf(container).constructor.namespace || (<any>target).namespace || 'Default';
        const containerTypeName = (<any>containerType).name;
    ...
    for (let i = 0; i < EdmContainer.length; i++) {
      const containerType = EdmContainer[i];
      const namespace = (<any>containerType).namespace || (<any>target).namespace || 'Default';
      const containerTypeName = (namespace ? namespace + '.' : '') + (<any>containerType).name;
      if (containerTypeName == elementType) {
...
function operationDecoratorFactory(type, returnType?) {
  return function (target, targetKey) {
    ...
    const existingOperations: any[] = Reflect.getOwnMetadata(EdmOperations, target) || [];
...
export function isEnumType(target: any, propertyKey?: string): boolean {
  return typeof propertyKey == 'string' ? Reflect.hasMetadata(EdmEnumType, target.prototype, propertyKey) : Reflect.hasMetadata(EdmEnumType, target);
}
...
export function getSerializer(target: Function, propertyKey: string, type?: any, container?: ContainerBase): Function {
  return (
    Reflect.getMetadata(EdmSerializer, target.prototype, propertyKey) ||
...
export function getDeserializer(target: Function, propertyKey: string, type?: any, container?: ContainerBase): Function {
  return (
    Reflect.getMetadata(EdmDeserializer, target.prototype, propertyKey) ||
...
export function getURLSerializer(target: Function, propertyKey: string, type?: any, container?: ContainerBase): Function {
  return (
    Reflect.getMetadata(EdmURLSerializer, target.prototype, propertyKey) ||
...
export function getURLDeserializer(target: Function, propertyKey: string, type?: any, container?: ContainerBase): Function {
  return (
    Reflect.getMetadata(EdmURLDeserializer, target.prototype, propertyKey) ||

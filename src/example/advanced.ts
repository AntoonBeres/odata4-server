...
export class ProductsController extends ODataController {
  ...
  async find(@odata.query query: ODataQuery, @odata.stream stream: Writable) {
    ...
    return (await mongodb())
      .collection('Products')
      .find(mongodbQuery.query, { projection: mongodbQuery.projection, skip: mongodbQuery.skip, limit: mongodbQuery.limit })
      .stream()
      .pipe(stream);
  }

  @odata.GET
  async findOne(@odata.key key: any, @odata.query query: ODataQuery) {
    const mongodbQuery = createQuery(query);
    return (await mongodb()).collection('Products').findOne({ _id: key }, mongodbQuery.projection);
  }
...
export class CategoriesController extends ODataController {
  ...
  async findOne(@odata.key() key: any, @odata.query query: ODataQuery) {
    const mongodbQuery = createQuery(query);
    return (await mongodb()).collection('Categories').findOne({ _id: key }, mongodbQuery.projection);
  }

...
describe('ODataValidation', () => {
  it('should throw validation error (@odata.query)', () => {
    return new Promise((resolve, reject) => {
      try {
        ValidationServer.execute('/Validation?$filter=Id eq 1')
          .then(
            () => {
              reject(new Error('should throw validation error'));
            },
            (err) => {
              if (err instanceof ValidationError) return resolve(undefined);
              reject(new Error('should throw validation error'));
            },
          )
          .catch((err) => {
  ...
  it('should throw validation error (@odata.filter)', () => {
    return new Promise((resolve, reject) => {
      try {
        ValidationServer.execute('/Validation(1)?$filter=Id eq 1')
          .then(
            () => {
              reject(new Error('should throw validation error'));
            },

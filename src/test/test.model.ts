...
export class ImagesController extends ODataController {
  ...
  getData2(@odata.key _: number, @odata.stream stream: Writable, @odata.context context: ODataHttpContext) {
    return new ODataStream(fs.createReadStream(path.join(__dirname, '..', '..', 'src', 'test', 'fixtures', 'logo_jaystack.png'))).pipe(
      context.response,
    );
  }
...
export class CategoriesGeneratorController extends ODataController {
  ...
  *find(@odata.filter filter: Token, @odata.stream stream: Writable) {
    ...
    if (filter)
      response = yield categories2.map((category) => Object.assign({}, category, { _id: category._id.toString() })).filter(createFilter(filter));
  ...
  *findProduct(@odata.key key: string, @odata.result result: Category2) {
    return yield products2.filter(
      (product) => product.CategoryId && product.CategoryId.toString() === result._id.toString() && product._id.toString() === key.toString(),
    );
  }
  ...
  *find(@odata.filter filter: Token) {
    if (filter) {
      return yield Promise.resolve(
        products2.map((product) => Object.assign({}, product, { _id: product._id.toString() })).filter(createFilter(filter)),
      );
    } else {
...
export class CategoriesPromiseGeneratorController extends ODataController {
  ...
  *find(@odata.filter filter: Token) {
    if (filter) {
      return yield Promise.resolve(
        categories2.map((category) => Object.assign({}, category, { _id: category._id.toString() })).filter(createFilter(filter)),
      );
    }
...
const getCategoriesByFilter = async (filter: Token) => {
  return await categories2.map((category) => Object.assign({}, category, { _id: category._id.toString() })).filter(createFilter(filter));
};
...
export class Images2Controller extends ODataController {
  ...
  @odata.GET('Data2')
  *getData2(@odata.key _: number, @odata.stream stream: Writable, @odata.context context: ODataHttpContext) {
    return yield new ODataStream(fs.createReadStream(path.join(__dirname, '..', '..', 'src', 'test', 'fixtures', 'logo_jaystack.png'))).pipe(
      context.response,
    );
  }
